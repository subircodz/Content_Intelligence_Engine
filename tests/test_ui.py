...
from intelligence_content_engine.ui import display_banner, display_pipeline_completion, display_welcome, prompt_user_topic
...
def test_prompt_user_topic_valid() -> None:
    with patch("intelligence_content_engine.ui.Prompt.ask", return_value="  My Custom Article Topic  "):
        assert prompt_user_topic() == "My Custom Article Topic"


def test_prompt_user_topic_retry_on_empty() -> None:
    with patch("intelligence_content_engine.ui.Prompt.ask", side_effect=["", "   ", "Valid Topic"]):
